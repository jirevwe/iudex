...
/** Summary data returned by collector */
...
/** Metadata returned by collector */
...
interface MockCollector {
  getSummary: jest.Mock<() => CollectorSummary>;
  getMetadata: jest.Mock<() => CollectorMetadata>;
  getAllResults: jest.Mock<() => CollectorResult[]>;
}

/** Mock repository for analytics */
interface MockRepository {
  getFlakyTests: jest.Mock;
  getRecentRegressions: jest.Mock;
  getTestHealthScores: jest.Mock;
  getDeletedTests: jest.Mock;
  searchTests?: jest.Mock;
}

describe('PostgresReporter', () => {
  let reporter: PostgresReporter;
  let mockCollector: MockCollector;
  let originalLogAdapter: string | undefined;
